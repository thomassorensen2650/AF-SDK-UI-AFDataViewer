...
namespace AFDataViewer
{
    public partial class MainForm : Form
    {
        ...
        private void btnAbout_Click(object sender, EventArgs e)
        {
            ...
            sb.AppendLine("Lots of Razzle-Dazzle with a Splitter, Row numbering in the grid, and a liberal use of bolding.");
            sb.AppendLine();
            sb.AppendLine("For Cancellation requests, we add some messaging to provide feedback to the user.");
            MessageBox.Show(this, sb.ToString(), "About Version 5", MessageBoxButtons.OK);
        }

        // CHANGED for Version 5
        private Panel LeftPanel => splitContainer1.Panel1;
        private Panel RightPanel => splitContainer1.Panel2;
        private void SetNitPickyAnchoringDetails()
        {
            // Left Panel: We set more Right anchors now that controls are assigned to a Panel rather than the form.
            piSystemPicker1.Anchor = AnchorStyles.Left | AnchorStyles.Right | AnchorStyles.Top;
            afDatabasePicker1.Anchor = AnchorStyles.Left | AnchorStyles.Right | AnchorStyles.Top;
            afElementFindCtrl1.Anchor = AnchorStyles.Left | AnchorStyles.Right | AnchorStyles.Top;
            afTreeView1.Anchor = AnchorStyles.Bottom | AnchorStyles.Left | AnchorStyles.Right | AnchorStyles.Top;

            // Right Panel: again with more Right anchor
            ...
            // Adjust heights of bottom-most controls in each panel, and use same margin from bottom.
            var margin = afTreeView1.Left;
            afTreeView1.Height = LeftPanel.Height - afTreeView1.Top - margin;
            dataGridView1.Height = RightPanel.Height - dataGridView1.Top - margin;

            // Change location of split container
            splitContainer1.Left = 0;
            splitContainer1.Top = 0;
            splitContainer1.Size = this.ClientSize;

            // And finally set its anchoring.
        ...
        // NEW to Version 4
        // We use a CancellationTokenSource rather than a CancellationToken in allow for cancellation requests.
        private CancellationTokenSource _cancellationTokenSource;
        private CancellationToken _cancelToken => (_cancellationTokenSource != null) ? _cancellationTokenSource.Token : CancellationToken.None;
        ...
        private void afTreeView1_AfterSelect(object sender, TreeViewEventArgs e)
        {
            ...
            if (SelectedAttribute != null)
            {
                ...
                ChangeColoring(lblSelectedAttrPath, Color.Black, null, true);
                ChangeColoring(lblTreeHeading, Color.Black, null, false);
            }
            else
            {
                lblSelectedAttrPath.Text = "Select an AFAttribute from the tree on the lower left";
                ChangeColoring(lblSelectedAttrPath, Color.Red, null, true);
                ChangeColoring(lblTreeHeading, Color.Red, null, true);
            }
        ...
        private void ChangeColoring(Control control, Color? forecolor, Color? backcolor, bool bold)
        {
            if (forecolor.HasValue) control.ForeColor = forecolor.Value;
            if (backcolor.HasValue) control.BackColor = backcolor.Value;
            control.Font = control.Font.WithStyle(bold ? FontStyle.Bold : FontStyle.Regular);
        }
        ...
        private async void btnGetData_Click(object sender, EventArgs e)
        {
            ...
            if (_wasCancelled)
            {
                MessageBox.Show(this, "Your request to cancel has been honored, though it may have been a bit sluggish.", "Operation Cancelled");
            }
        }
        ...
        private async Task<AFValues> RetrieveDataAsync(int dataMethodIndex, AFTimeRange timeRange)
        {
            ...
            {
                if (IgnoreUserCancelException(ex))
                {
                    _wasCancelled = canBeCancelled && _cancelToken.IsCancellationRequested;
                }
                else
                {
                    throw;
        ...
        private async Task<AFValues> GetValuesAsync(AFAttribute attribute, AFTimeRange timeRange)
        {
            ...
            await Task.Factory.StartNew(() =>
            {
                // Despite passing a cancellation token, it's left to our code to check
                // for it and gracefully stop processing.  The problem is there is NO place
                // to check.  We must wait for GetValues to completely finish in order to 
                // stop and by then you may as well publish the results.
                //
                // So why wrap the GetValues call in this task?  The only good reason is
        ...
        private void TimeBoxValidate(TextBox textBox)
        {
            ...
            {
                ChangeColoring(textBox, Color.Black, Color.White, false);
            }
            else
            {
                ChangeColoring(textBox, Color.Red, Color.Yellow, true);
            }
            CheckUserInputsState();
        }
        ...
        private void cbxDataMethods_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            if (supported)
            {
                ChangeColoring(cbxDataMethods, Color.Black, Color.White, false);
            }
            else
            {
                ChangeColoring(cbxDataMethods, Color.Red, Color.Yellow, true);
            }
        ...
        // NEW To Version 4
        #region "Dynamic Cancel"

        private Button btnRequestCancellation = new Button();

        private void CreateCancelButton()
        {
            btnRequestCancellation.Name = "btnRequestCancellation";
            btnRequestCancellation.Text = "Cancel";
            btnRequestCancellation.Visible = false;

            btnRequestCancellation.Font = btnRequestCancellation.Font.WithStyle(FontStyle.Bold);
            btnRequestCancellation.ForeColor = Color.Maroon;

            btnRequestCancellation.Parent = btnGetData.Parent;
            btnRequestCancellation.Size = btnGetData.Size;
            btnRequestCancellation.Location = btnGetData.Location;
            btnRequestCancellation.Anchor = btnGetData.Anchor;

            // Assign an event handler 
            btnRequestCancellation.Click += btnRequestCancellation_Click;
        }

        private void btnRequestCancellation_Click(object sender, EventArgs e)
        {
            if (_cancellationTokenSource != null && !_cancelToken.IsCancellationRequested)
            {
                // Note we don't actually cancel.  Rather we communicate a request that we would like to cancel.
                // It's left to the affected tasks to check it's token's IsCancellationRequested property and
                // gracefully stop itself - which won't happen with GetValues() since that is a monolithic call.
                // Lesson learned: whenever possible, you want to use RecordedValuesAsync instead of GetValues().
                _cancellationTokenSource.Cancel(true);

                if (cbxDataMethods.SelectedIndex == GetValuesIndex)
                {
                    MessageBox.Show(this, "Even though GetValues is running in a background thread, GetValues does not have an Async method and therefore cannot be cancelled.", "I'm gonna let you finish...");
                }

                // And you only need to click on this ONCE to cancel.
                btnRequestCancellation.Enabled = false;
            }
        }

        private void ShowCancelButton(bool activate)
        {
            btnRequestCancellation.Enabled = activate;
            btnRequestCancellation.Visible = activate;
            btnGetData.Visible = !activate;
        }

#endregion

        // NEW to Version 4
        private bool IgnoreUserCancelException(Exception exception)
        {
            ...
            if (aggregateException != null)
            {
                ...
                {
                    if (!IgnoreUserCancelException(innerException))
                    {
                        return false;
                    }
                    return true;
        ...
        // NEW to Version 5.  See accompanying extension method.
        private void dataGridView1_RowPostPaint(object sender, DataGridViewRowPostPaintEventArgs e)
        {
            dataGridView1.PostPaintRowNumber(e);
        }
